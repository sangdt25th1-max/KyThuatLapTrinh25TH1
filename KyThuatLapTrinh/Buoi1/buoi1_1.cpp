...
void ViewPresonList(vector<Person> p) {
	if (p.size() == 0)
...
void RemovePerson(vector<Person>& p, int id) {
	for (auto i = p.begin(); i != p.end(); i++) {
		if (i->id == id) {
			p.erase(i);
			cout << "Remove a person successfuly" << endl;
			return;
		}
	}
}
int main() {
	...
	do {
		...
		cout << "1.View preson list" << endl;
		cout << "2.Add a preson " << endl;
		cout << "3. Remove a preson" << endl;
		cout << "4.Find a preson by name" << endl;
